...
export default function TicTacToeClient() {
  ...
  async function placeMark(index: number) {
    ...
    const data = (await res.json()) as RoomApi | ApiError;
    if (!res.ok || "error" in data) {
      setError("error" in data ? toUiError(data.error, "Move failed. Check if it is your turn.") : "Move failed. Check if it is your turn.");
      return;
    }
  ...
  async function sendAction(action: Record<string, unknown>) {
    ...
    const data = (await res.json()) as RoomApi | ApiError;
    if (!res.ok || "error" in data) {
      setError("error" in data ? toUiError(data.error, "Action failed. Please try again.") : "Action failed. Please try again.");
      return;
    }
